...
**Production URL**: `https://api.youtubesummaries.cc`
...
```javascript
const API_BASE_URL = 'https://api.youtubesummaries.cc';
```
...
async function getVideoMetadata(videoId) {
  try {
    const response = await fetch(
      `https://api.youtubesummaries.cc/youtube/metadata?video=${encodeURIComponent(videoId)}`
    );
...
async function getVideoCaptions(videoId, languages = ['en']) {
  try {
    ...
    const response = await fetch(
      `https://api.youtubesummaries.cc/youtube/captions?${params}`
    );
...
async function getVideoTimestamps(videoId, languages = ['en']) {
  try {
    ...
    const response = await fetch(
      `https://api.youtubesummaries.cc/youtube/timestamps?${params}`
    );
...
async function getCacheStats() {
  const response = await fetch(
    'https://api.youtubesummaries.cc/youtube/cache/stats'
  );
...
async function clearCache() {
  const response = await fetch(
    'https://api.youtubesummaries.cc/youtube/cache/clear',
    { method: 'DELETE' }
  );
  return await response.json();
}
...
async function testPerformance(videoId, runs = 5) {
  ...
  const response = await fetch(
    `https://api.youtubesummaries.cc/youtube/performance/test?${params}`
  );
...
try {
  const metadata = await safeApiCall(
    'https://api.youtubesummaries.cc/youtube/metadata?video=invalid'
  );
} catch (error) {
...
```javascript
class YouTubeAPI {
  constructor(baseURL = 'https://api.youtubesummaries.cc') {
...
const url = `https://api.youtubesummaries.cc/youtube/metadata?video=${videoId}`;

// ❌ Bad
const url = `https://api.youtubesummaries.cc/youtube/metadata?video=dQw4w9WgXcQ`;
```
...
async function testConnection() {
  try {
    const status = await fetch('https://api.youtubesummaries.cc/health');
...
python test_transcript_speed.py https://api.youtubesummaries.cc
...
- **API Documentation**: `https://api.youtubesummaries.cc/docs`
- **Service Status**: `https://api.youtubesummaries.cc/service/info`
